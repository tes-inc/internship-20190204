...
function display() { // 保存されたデータを表示する関数
  ...
  for (const item in localArray) { // JSON形式のデータを一つずつ取り出して表示
    $("#recordedData").append(`
      <span id = "recordedTitle">『${localArray[item]["title"]}』</span>
      <span>  ☆${localArray[item]["rate"]}</span>
      <span>  投稿者名：${localArray[item]["name"]}</span>
      <div>オススメ理由：${localArray[item]["review"]}</div><br><hr>
      `);
  }
}